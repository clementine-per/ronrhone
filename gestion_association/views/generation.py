...
from gestion_association.models.animal import Animal, SexeChoice, TrancheAge

# Couleurs et styles pour la génération PDF
couleur_ronrhone = "#00bfff"
blackParagraphStyle = ParagraphStyle(
    name="Black",
    textColor="black",
...
redParagraphStyle = ParagraphStyle(
    name="Red",
    textColor="red",
    alignement=TA_JUSTIFY,
    fontSize=0.5 * cm,
...
blueParagraphStyle = ParagraphStyle(
    name="Blue",
    textColor=couleur_ronrhone,
    alignement=TA_JUSTIFY,
    fontSize=0.5 * cm,
    fontName="Helvetica",
    borderPadding=(0.01 * cm, 1 * cm, 0.5 * cm, 2.7 * cm),
...
def generate_contract(request, pk):
    ...
    certificat_engagement = open(f"{settings.STATIC_ROOT}/pdf/certificat-engagement.pdf", "rb")
...
def generation_reglement(p, difference, spaceStyle, animal):
    ...
    para = Paragraph(
        "<font face='times-bold' size=14><u>Paylib ou Lydia :</u>  </font><br/> \
...
def en_tete(p, is_enfant, animal):
    ...
    para = Paragraph(f"Contrat d'adoption de  <br/><br/> {animal.nom} <br/>", style=style)
    para.wrap(8 * cm, 7 * cm)
...
def infos_personnelles(p, animal):
    ...
    para = Paragraph("Informations personnelles de l'adoptant <br/>", style=titleStyle)
    para.wrap(14 * cm, 15 * cm)
...
def infos_pensionnaire(p, animal, is_enfant):
    para = Paragraph(f'{"Informations sur le pensionnaire adopté"} <br/>', style=titleStyle)
    para.wrap(14 * cm, 15 * cm)
    ...
    p.drawString(
        2 * cm, 9.55 * cm, "- Date de naissance : " + animal.date_naissance.strftime("%d/%m/%Y")
    )
...
def info_tarifs_adulte(p):
    elements = [
        [
            Paragraph("Chat femelle Primo"),
            Paragraph(
                "Identification + Stérilisation + Primo vaccin + Test FIV/FELV + Déparasitant + Vermifuge"
            ),
            "170€",
        ],
        [
            Paragraph("Chat femelle Primo + Rappel"),
            Paragraph(
        ...
        [
            ...
            Paragraph(
                "Identification + Castration + Primo vaccin + Test FIV/FELV + Déparasitant + Vermifuge"
            ),
            "150€",
        ],
        [
            ...
            Paragraph(
                "Identification + Castration + Vaccins à jour + Test FIV/FELV + Déparasitant + Vermifuge"
            ),
            "180€",
        ],
    ]
...
def info_rappel_vaccin(p, animal):
    ...
    if animal.vaccin_ok == OuiNonChoice.OUI.name:
        p.drawString(5.5 * cm, 27.5 * cm, "peu de temps avant le " + prochain_vaccin_str + ".")
    elif prochain_vaccin:
        vaccin_delai = prochain_vaccin + relativedelta(days=7)
        p.drawString(
            5.5 * cm,
            27.5 * cm,
            ((f"entre le {prochain_vaccin_str} et le " + vaccin_delai.strftime("%d/%m/%Y")) + "."),
        )
...
def info_sterilisation(p, spaceStyle, animal):
    ...
    para = Paragraph(
        "<font face='helvetica' size=12>Si l’Association ne reçoit pas de certificat de stérilisation avant <br/> \
                        ...
                        par l’adoptant et d’encaisser le chèque de caution éventuel</font>".format(
            (animal.date_naissance + relativedelta(months=7)).strftime("%d/%m/%Y")
        )
    )
    para.wrap(14 * cm, 15 * cm)
    ...
    para = Paragraph(
        "<font face='helvetica' size=12>Environ deux mois après l’adoption, l’Association prendra rendez-vous <br/> \
...
def exigences_alimentaires(p, animal, vertical):
    ...
    para = Paragraph(
        (
            f"Le chat susnommé {animal.nom}" + " devra être nourri avec une alimentation répondant"
...
def engagement(p, animal, vertical):
    ...
    p.setFillColor("red")
    p.drawString(
        1.5 * cm, (vertical - 6) * cm, "ATTENTION ! AUCUN CHAT NE DOIT AVOIR ACCES" " A UN ESPACE"
    )
...
def montants(p, animal, vertical):
    ...
    if montant_bon:
        p.drawString(
            9.4 * cm,
            (vertical + 4.5) * cm,
            str(animal.get_latest_adoption().montant - montant_bon),
        )
    else:
    ...
    if animal.get_latest_adoption().montant:
        p.drawString(
            9.4 * cm,
            (vertical + 3.8) * cm,
            str(animal.get_latest_adoption().montant - montant_restant),
        )
    if montant_bon:
